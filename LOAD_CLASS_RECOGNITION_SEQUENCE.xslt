...
<xsl:param name="delimiter" select=" '|' "/>
	...
	<xsl:template match="/">
	<xsl:text>Base|Original Sequence|Sequence|Complement|Length|Specificity Length|Is Palindromic|Is Degenerate|Position on Sequence|Position on Complement|Over Hang Sequence|Over Hang Description|Min Adjustment|Max Adjustment|OverHang Length</xsl:text>
<xsl:text>&#13;&#10;</xsl:text>
...
<xsl:value-of select="$delimiter"/>
<xsl:value-of select="normalize-space(SpecificityLength)"/>
<xsl:value-of select="$delimiter"/>
<xsl:value-of select="normalize-space(IsPalindromic)"/>
<xsl:value-of select="$delimiter"/>
<xsl:value-of select="normalize-space(IsDegenerate)"/>
<xsl:value-of select="$delimiter"/>
<xsl:value-of select="normalize-space(PositionOnSequence)"/>
<xsl:value-of select="$delimiter"/>
<xsl:value-of select="normalize-space(PositionOnComplement)"/>
<xsl:value-of select="$delimiter"/>
<xsl:value-of select="normalize-space(OverhangSequence)"/>
<xsl:value-of select="$delimiter"/>
